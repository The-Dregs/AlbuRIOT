...
public class AmomongoAI : MonoBehaviourPun, IEnemyDamageable
{
    [Header("stats (inline)")]
    ...
    private void BuildTree()
    {
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "berserk_seq").Add(canBerserk, doBerserk),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[AmomongoAI] Starting basic attack on {t.name}");
    ...
    private bool CanDoSlam()
    {
        ...
        return d <= slamRadius + 2f; // prefer slam if target is close enough
    }
    ...
    private System.Collections.IEnumerator SlamRoutine()
    {
        ...
        // apply AOE damage
        float damage = isBerserk ? slamDamageBerserk : slamDamage;
        var cols = Physics.OverlapSphere(transform.position, slamRadius, LayerMask.GetMask("Player"));
        foreach (var c in cols)
        {
            var ps = c.GetComponentInParent<PlayerStats>();
            if (ps != null)
                DamageRelay.ApplyToPlayer(ps.gameObject, (int)damage);
        }
        float rec = Mathf.Max(0.1f, attackMoveLock);
        while (rec > 0f && (timeout -= Time.deltaTime) > 0f)
        {
    ...
    private NodeState DoBerserk()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastBerserkTime = Time.time;
        activeAbility = StartCoroutine(BerserkRoutine());
        return NodeState.Success;
    }
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("AmomongoDebugText");
