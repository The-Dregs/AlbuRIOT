...
public class BusawAI : MonoBehaviourPun
{
    [Header("stats")]
    public int maxHealth = 180;
    public int basicDamage = 22;
    public float basicRange = 2.2f;
    public float basicCooldown = 2.1f;
    public float basicWindup = 0.45f;

    [Header("Corpse Feast")]
    public int feastHeal = 40;
    public float feastRange = 3.5f;
    public float feastCooldown = 10f;
    public float feastWindup = 1.1f;
    public LayerMask corpseLayer;

    [Header("Graveyard Grasp")]
    public int graspDamage = 30;
    public float graspRange = 4.5f;
    public float graspCooldown = 8f;
    public float graspWindup = 0.8f;
    public float graspRootDuration = 2.5f;
    public float graspRadius = 2.5f;
    ...
    private float lastFeastTime;
    private float lastGraspTime;
    private float attackLockTimer;
    private bool isDead;
    private bool isFeasting;
    private bool isGrasping;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isFeasting || isGrasping || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Feast, Grasp }
    ...
    public float FeastCooldownRemaining => Mathf.Max(0f, feastCooldown - (Time.time - lastFeastTime));
    ...
    private void BuildTree()
    {
        ...
        var canFeast = new ConditionNode(bb, ()=>{ return CanDoFeast(); }, "can_feast");
        var doFeast = new ActionNode(bb, ()=>{ currentState = "feast windup"; return DoFeast(); }, "do_feast");
        var canGrasp = new ConditionNode(bb, ()=>{ return CanDoGrasp(); }, "can_grasp");
        var doGrasp = new ActionNode(bb, ()=>{ currentState = "grasp windup"; return DoGrasp(); }, "do_grasp");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "feast_seq").Add(canFeast, doFeast),
            new Sequence(bb, "grasp_seq").Add(canGrasp, doGrasp),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isFeasting || isGrasping || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[BusawAI] Starting basic attack on {t.name}");
    ...
    private bool CanDoFeast()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isFeasting) return false;
        if (Time.time - lastFeastTime < feastCooldown) return false;
        // check for corpse in range
        var corpses = Physics.OverlapSphere(transform.position, feastRange, corpseLayer);
        return corpses.Length > 0;
    }

    private NodeState DoFeast()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastFeastTime = Time.time;
        activeAbility = StartCoroutine(FeastRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator FeastRoutine()
    {
        currentAction = ActionState.Feast;
        isFeasting = true;
        float wind = feastWindup;
        float timeout = abilityHardTimeout;
        ...
        foreach (var c in corpses)
        {
            ...
            currentHealth = Mathf.Min(currentHealth + feastHeal, maxHealth);
    ...
    private bool CanDoGrasp()
    {
        ...
        return d <= graspRange + 2f;
    }

    private NodeState DoGrasp()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastGraspTime = Time.time;
        activeAbility = StartCoroutine(GraspRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator GraspRoutine()
    {
        currentAction = ActionState.Grasp;
        isGrasping = true;
        float wind = graspWindup;
        float timeout = abilityHardTimeout;
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("BusawDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdFeast = FeastCooldownRemaining;
            float cdGrasp = GraspCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} feast:{(isFeasting?"Y":"N")} grasp:{(isGrasping?"Y":"N")}\ncd B:{cdBasic:F1} F:{cdFeast:F1} G:{cdGrasp:F1}";
        }
