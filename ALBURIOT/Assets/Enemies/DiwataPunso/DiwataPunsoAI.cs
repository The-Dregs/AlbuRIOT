...
public class DiwataPunsoAI : MonoBehaviourPun
{
    [Header("stats")]
    public int maxHealth = 160;
    public int basicDamage = 18;
    public float basicRange = 2.1f;
    public float basicCooldown = 2.0f;
    public float basicWindup = 0.4f;

    [Header("Roots")]
    public int rootsDamage = 24;
    public float rootsRange = 4.0f;
    public float rootsCooldown = 7.5f;
    public float rootsWindup = 0.7f;
    public float rootsRootDuration = 2.0f;
    public float rootsRadius = 2.5f;

    [Header("Nature Bolt")]
    public int boltDamage = 32;
    public float boltRange = 8.0f;
    public float boltCooldown = 9.5f;
    public float boltWindup = 0.9f;
    public float boltProjectileSpeed = 14f;
    ...
    private bool IsBusy => activeAbility != null || isRooting || isBolting || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Roots, Bolt }
    ...
    public float RootsCooldownRemaining => Mathf.Max(0f, rootsCooldown - (Time.time - lastRootsTime));
    public float BoltCooldownRemaining => Mathf.Max(0f, boltCooldown - (Time.time - lastBoltTime));
    ...
    private void BuildTree()
    {
        ...
        var canRoots = new ConditionNode(bb, ()=>{ return CanDoRoots(); }, "can_roots");
        var doRoots = new ActionNode(bb, ()=>{ currentState = "roots windup"; return DoRoots(); }, "do_roots");
        var canBolt = new ConditionNode(bb, ()=>{ return CanDoBolt(); }, "can_bolt");
        var doBolt = new ActionNode(bb, ()=>{ currentState = "bolt windup"; return DoBolt(); }, "do_bolt");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "roots_seq").Add(canRoots, doRoots),
            new Sequence(bb, "bolt_seq").Add(canBolt, doBolt),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isRooting || isBolting || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[DiwataPunsoAI] Starting basic attack on {t.name}");
    ...
    private bool CanDoRoots()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isRooting) return false;
        if (Time.time - lastRootsTime < rootsCooldown) return false;
        var t = bb.Get<Transform>("target");
        if (t == null) return false;
        float d = Vector3.Distance(transform.position, t.position);
        return d <= rootsRange + 2f;
    }

    private NodeState DoRoots()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastRootsTime = Time.time;
        activeAbility = StartCoroutine(RootsRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator RootsRoutine()
    {
        currentAction = ActionState.Roots;
        isRooting = true;
        float wind = rootsWindup;
        float timeout = abilityHardTimeout;
        ...
        // AOE root/damage
        var cols = Physics.OverlapSphere(transform.position, rootsRadius, LayerMask.GetMask("Player"));
        foreach (var c in cols)
        {
            var ps = c.GetComponentInParent<PlayerStats>();
            if (ps != null)
    ...
    private bool CanDoBolt()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isBolting) return false;
        if (Time.time - lastBoltTime < boltCooldown) return false;
        var t = bb.Get<Transform>("target");
        if (t == null) return false;
        float d = Vector3.Distance(transform.position, t.position);
        return d <= boltRange + 2f;
    }

    private NodeState DoBolt()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastBoltTime = Time.time;
        activeAbility = StartCoroutine(BoltRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator BoltRoutine()
    {
        currentAction = ActionState.Bolt;
        isBolting = true;
        float wind = boltWindup;
        float timeout = abilityHardTimeout;
        ...
        // projectile
        if (boltPrefab != null && t != null)
        {
            var go = Instantiate(boltPrefab, transform.position + transform.forward * 1.2f, Quaternion.identity);
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("DiwataPunsoDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdRoots = RootsCooldownRemaining;
            float cdBolt = BoltCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} roots:{(isRooting?"Y":"N")} bolt:{(isBolting?"Y":"N")}\ncd B:{cdBasic:F1} R:{cdRoots:F1} N:{cdBolt:F1}";
        }
