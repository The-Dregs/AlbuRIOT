...
public class MinokawaFragmentAI : MonoBehaviourPun
{
    [Header("stats")]
    public int maxHealth = 120;
    public int basicDamage = 14;
    public float basicRange = 1.8f;
    public float basicCooldown = 1.7f;
    public float basicWindup = 0.35f;

    [Header("Solar Flare")]
    public int flareDamage = 22;
    public float flareRange = 5.0f;
    public float flareCooldown = 7.5f;
    public float flareWindup = 0.7f;
    public float flareRadius = 2.2f;

    [Header("Blazing Dive")]
    public int diveDamage = 26;
    public float diveRange = 6.0f;
    public float diveCooldown = 8.5f;
    public float diveWindup = 0.8f;
    public float diveMoveSpeed = 7.0f;
    ...
    public string flareTrigger = "Flare";
    ...
    private bool isDead;
    private bool isFlareing;
    private bool isDiving;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isFlareing || isDiving || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Flare, Dive }
    ...
    public float FlareCooldownRemaining => Mathf.Max(0f, flareCooldown - (Time.time - lastFlareTime));
    ...
    private void BuildTree()
    {
        ...
        var canFlare = new ConditionNode(bb, ()=>{ return CanDoFlare(); }, "can_flare");
        var doFlare = new ActionNode(bb, ()=>{ currentState = "flare windup"; return DoFlare(); }, "do_flare");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "flare_seq").Add(canFlare, doFlare),
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isFlareing || isDiving || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[MinokawaFragmentAI] Starting basic attack on {t.name}");
    ...
    private bool CanDoFlare()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isFlareing) return false;
        if (Time.time - lastFlareTime < flareCooldown) return false;
        var t = bb.Get<Transform>("target");
        if (t == null) return false;
        float d = Vector3.Distance(transform.position, t.position);
        return d <= flareRange + 2f;
    }

    private NodeState DoFlare()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastFlareTime = Time.time;
        activeAbility = StartCoroutine(FlareRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator FlareRoutine()
    {
        currentAction = ActionState.Flare;
        isFlareing = true;
        float wind = flareWindup;
        float timeout = abilityHardTimeout;
        if (animator != null && HasTrigger(flareTrigger)) animator.SetTrigger(flareTrigger);
        var t = bb.Get<Transform>("target");
        ...
        // AOE
        var cols = Physics.OverlapSphere(transform.position, flareRadius, LayerMask.GetMask("Player"));
        foreach (var c in cols)
        {
            var ps = c.GetComponentInParent<PlayerStats>();
            if (ps != null)
                DamageRelay.ApplyToPlayer(ps.gameObject, flareDamage);
        }
        isFlareing = false;
        float rec = Mathf.Max(0.1f, attackMoveLock);
        while (rec > 0f && (timeout -= Time.deltaTime) > 0f)
    ...
    private System.Collections.IEnumerator DiveRoutine()
    {
        ...
        if (t != null) FaceTarget(t);
        // dash
        float dashTimer = 0.5f + (diveMoveSpeed / 10f);
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("MinokawaFragmentDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdFlare = FlareCooldownRemaining;
            float cdDive = DiveCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} flare:{(isFlareing?"Y":"N")} dive:{(isDiving?"Y":"N")}\ncd B:{cdBasic:F1} F:{cdFlare:F1} D:{cdDive:F1}";
        }
