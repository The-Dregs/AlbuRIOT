...
public class PugotAI : MonoBehaviourPun
{
    ...
    public int basicDamage = 16;
    public float basicRange = 2.0f;
    public float basicCooldown = 1.9f;
    public float basicWindup = 0.4f;

    [Header("Head Throw")]
    public int throwDamage = 24;
    public float throwRange = 6.0f;
    public float throwCooldown = 8.5f;
    public float throwWindup = 0.8f;
    public GameObject headProjectilePrefab;
    public float throwProjectileSpeed = 12f;

    [Header("Grave Step")]
    public int stepDamage = 20;
    public float stepRange = 4.0f;
    public float stepCooldown = 7.5f;
    public float stepWindup = 0.7f;
    public float stepRootDuration = 1.8f;
    public float stepRadius = 2.0f;

    [Header("movement: speed settings")]
    public float patrolSpeed = 1.8f;
    public float chaseSpeed = 3.8f;
    public float attackMoveLock = 0.4f;
    public bool orbitWhenBasicOnCooldown = true;
    [Range(0.1f, 1.5f)] public float orbitSpeedMultiplier = 0.6f;
    ...
    private float lastHeadThrowTime;
    private float lastGraveStepTime;
    private float attackLockTimer;
    private bool isDead;
    private bool isThrowingHead;
    private bool isGraveStepping;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isThrowingHead || isGraveStepping || attackLockTimer > 0f;
    private enum ActionState { None, Basic, HeadThrow, GraveStep }
    private ActionState currentAction = ActionState.None;
    public float abilityHardTimeout = 8f;
    [Header("global ability gating")]
    public float postSpecialLock = 0.8f;
    private float postSpecialTimer = 0f;

    // debug overlay
    [Header("debug overlay")]
    public bool showDebugOverhead = false;
    public float debugYOffset = 2.2f;
    ...
    private void Awake()
    {
        ...
        lastBasicTime = -9999f;
        lastHeadThrowTime = -9999f;
        lastGraveStepTime = -9999f;
    ...
    private void Update()
    {
        ...
        if (tgt != null && Vector3.Distance(transform.position, tgt.position) > 16f)
        {
    ...
    private void BuildTree()
    {
        ...
        var canHeadThrow = new ConditionNode(bb, ()=>{ return CanDoHeadThrow(); }, "can_head_throw");
        var doHeadThrow = new ActionNode(bb, ()=>{ currentState = "head throw windup"; return DoHeadThrow(); }, "do_head_throw");
        var canGraveStep = new ConditionNode(bb, ()=>{ return CanDoGraveStep(); }, "can_grave_step");
        var doGraveStep = new ActionNode(bb, ()=>{ currentState = "grave step windup"; return DoGraveStep(); }, "do_grave_step");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "head_throw_seq").Add(canHeadThrow, doHeadThrow),
            new Sequence(bb, "grave_step_seq").Add(canGraveStep, doGraveStep),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isThrowingHead || isGraveStepping || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[PugotAI] Starting basic attack on {t.name}");
    ...
    private NodeState DoHeadThrow()
    {
        ...
        if (activeAbility != null) return NodeState.Running;
        lastHeadThrowTime = Time.time;
        activeAbility = StartCoroutine(HeadThrowRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator HeadThrowRoutine()
    {
    ...
    float wind = throwWindup;
        ...
        if (t != null)
        {
            ...
            Vector3 end = t.position + Vector3.up * 1.2f;
            Vector3 dir = (end - start).normalized;
            float dist = Vector3.Distance(start, end);
            RaycastHit hit;
            if (Physics.Raycast(start, dir, out hit, dist + 1f, LayerMask.GetMask("Player")))
            {
    ...
    private NodeState DoGraveStep()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastGraveStepTime = Time.time;
        activeAbility = StartCoroutine(GraveStepRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator GraveStepRoutine()
    {
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("PugotDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdHead = HeadThrowCooldownRemaining;
            float cdGrave = GraveStepCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} head:{(isThrowingHead?"Y":"N")} grave:{(isGraveStepping?"Y":"N")}\ncd B:{cdBasic:F1} H:{cdHead:F1} G:{cdGrave:F1}";
        }
