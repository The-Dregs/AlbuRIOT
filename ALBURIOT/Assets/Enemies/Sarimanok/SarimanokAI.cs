...
public class SarimanokAI : MonoBehaviourPun
{
    ...
    public int basicDamage = 15;
    public float basicRange = 1.9f;
    public float basicCooldown = 1.8f;
    public float basicWindup = 0.35f;

    [Header("Despair Cry")]
    public int cryDamage = 18;
    public float cryRange = 5.0f;
    public float cryCooldown = 7.5f;
    public float cryWindup = 0.7f;
    public float cryFearDuration = 2.0f;
    public float cryRadius = 2.2f;

    [Header("Blazing Feathers")]
    public int featherDamage = 22;
    public float featherRange = 6.0f;
    public float featherCooldown = 8.5f;
    public float featherWindup = 0.8f;
    public GameObject featherProjectilePrefab;
    public float featherProjectileSpeed = 13f;
    ...
    [Range(0.1f, 1.5f)] public float orbitSpeedMultiplier = 0.8f;

    [Header("animation")]
    public Animator animator;
    public string speedParam = "Speed";
    public string attackTrigger = "Attack";
    public string despairCryTrigger = "DespairCry";
    public string blazingFeathersTrigger = "BlazingFeathers";
    ...
    private float lastDespairCryTime;
    private float lastBlazingFeathersTime;
    private float attackLockTimer;
    private bool isDead;
    private bool isCrying;
    private bool isFeathering;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isCrying || isFeathering || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Cry, Feathers }
    ...
    public float debugYOffset = 2.6f;
    ...
    private void Awake()
    {
        ...
        lastBasicTime = -9999f;
        lastDespairCryTime = -9999f;
        lastBlazingFeathersTime = -9999f;
    ...
    private void BuildTree()
    {
        ...
        var canCry = new ConditionNode(bb, ()=>{ return CanDoDespairCry(); }, "can_cry");
        var doCry = new ActionNode(bb, ()=>{ currentState = "cry windup"; return DoDespairCry(); }, "do_cry");
        var canFeathers = new ConditionNode(bb, ()=>{ return CanDoBlazingFeathers(); }, "can_feathers");
        var doFeathers = new ActionNode(bb, ()=>{ currentState = "feathers windup"; return DoBlazingFeathers(); }, "do_feathers");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "cry_seq").Add(canCry, doCry),
            new Sequence(bb, "feathers_seq").Add(canFeathers, doFeathers),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isCrying || isFeathering || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[SarimanokAI] Starting basic attack on {t.name}");
    ...
    private NodeState DoDespairCry()
    {
        ...
        if (activeAbility != null) return NodeState.Running;
        lastDespairCryTime = Time.time;
        activeAbility = StartCoroutine(DespairCryRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator DespairCryRoutine()
    {
    ...
    private bool CanDoBlazingFeathers()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
    ...
    private NodeState DoBlazingFeathers()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastBlazingFeathersTime = Time.time;
        activeAbility = StartCoroutine(BlazingFeathersRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator BlazingFeathersRoutine()
    {
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("SarimanokDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdCry = DespairCryCooldownRemaining;
            float cdFeathers = BlazingFeathersCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} cry:{(isCrying?"Y":"N")} feathers:{(isFeathering?"Y":"N")}\ncd B:{cdBasic:F1} C:{cdCry:F1} F:{cdFeathers:F1}";
        }
