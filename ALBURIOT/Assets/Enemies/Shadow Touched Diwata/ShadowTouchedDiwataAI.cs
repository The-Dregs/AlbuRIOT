...
public class ShadowTouchedDiwataAI : MonoBehaviourPun
{
    ...
    [Header("Eclipse Veil")]
    public int veilDamage = 28;
    public float veilRange = 7.0f;
    public float veilCooldown = 9.5f;
    ...
    public float lamentRange = 6.0f;
    public float lamentCooldown = 11f;
    public float lamentWindup = 1.2f;
    public float lamentFearDuration = 2.5f;
    public float lamentRadius = 3.0f;
    ...
    public string eclipseVeilTrigger = "EclipseVeil";
    public string lamentTrigger = "Lament";
    ...
    private float lastEclipseVeilTime;
    private float lastLamentTime;
    private float attackLockTimer;
    private bool isDead;
    private bool isVeiling;
    private bool isLamenting;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isVeiling || isLamenting || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Veil, Lament }
    ...
    private void Awake()
    {
        ...
        lastBasicTime = -9999f;
        lastEclipseVeilTime = -9999f;
        lastLamentTime = -9999f;
    ...
    private void BuildTree()
    {
        ...
        var canVeil = new ConditionNode(bb, ()=>{ return CanDoEclipseVeil(); }, "can_veil");
        var doVeil = new ActionNode(bb, ()=>{ currentState = "veil windup"; return DoEclipseVeil(); }, "do_veil");
        var canLament = new ConditionNode(bb, ()=>{ return CanDoLament(); }, "can_lament");
        var doLament = new ActionNode(bb, ()=>{ currentState = "lament windup"; return DoLament(); }, "do_lament");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "veil_seq").Add(canVeil, doVeil),
            new Sequence(bb, "lament_seq").Add(canLament, doLament),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isVeiling || isLamenting || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[ShadowTouchedDiwataAI] Starting basic attack on {t.name}");
    ...
    private NodeState DoEclipseVeil()
    {
        ...
        activeAbility = StartCoroutine(EclipseVeilRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator EclipseVeilRoutine()
    {
    ...
    private bool CanDoLament()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isLamenting) return false;
        if (Time.time - lastLamentTime < lamentCooldown) return false;
        var t = bb.Get<Transform>("target");
        if (t == null) return false;
        float d = Vector3.Distance(transform.position, t.position);
        return d <= lamentRange + 2f;
    }

    private NodeState DoLament()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastLamentTime = Time.time;
        activeAbility = StartCoroutine(LamentRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator LamentRoutine()
    {
        currentAction = ActionState.Lament;
        isLamenting = true;
        float wind = lamentWindup;
        float timeout = abilityHardTimeout;
        if (animator != null && HasTrigger(lamentTrigger)) animator.SetTrigger(lamentTrigger);
        var t = bb.Get<Transform>("target");
        if (t != null) FaceTarget(t);
        while (wind > 0f && (timeout -= Time.deltaTime) > 0f)
        {
        ...
        // radial DoT/fear
        var cols = Physics.OverlapSphere(transform.position, lamentRadius, LayerMask.GetMask("Player"));
        foreach (var c in cols)
        {
            var ps = c.GetComponentInParent<PlayerStats>();
            if (ps != null)
            {
                DamageRelay.ApplyToPlayer(ps.gameObject, lamentDamage);
                // TODO: apply DoT/fear effect via PlayerStatusRelay
            }
        }
        isLamenting = false;
        float rec = Mathf.Max(0.1f, attackMoveLock);
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("ShadowTouchedDiwataDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdVeil = EclipseVeilCooldownRemaining;
            float cdLament = LamentCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} veil:{(isVeiling?"Y":"N")} lament:{(isLamenting?"Y":"N")}\ncd B:{cdBasic:F1} V:{cdVeil:F1} L:{cdLament:F1}";
        }
