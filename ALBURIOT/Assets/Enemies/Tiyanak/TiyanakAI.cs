...
public class TiyanakAI : MonoBehaviourPun
{
    [Header("stats")]
    public int maxHealth = 110;
    public int basicDamage = 12;
    public float basicRange = 1.5f;
    public float basicCooldown = 1.5f;
    public float basicWindup = 0.3f;

    [Header("Lunge Bite")]
    public int biteDamage = 16;
    public float biteRange = 3.5f;
    public float biteCooldown = 6.5f;
    public float biteWindup = 0.5f;
    public float biteMoveSpeed = 6.0f;

    [Header("Wail Fear")]
    public int wailDamage = 14;
    public float wailRange = 4.5f;
    public float wailCooldown = 7.0f;
    public float wailWindup = 0.6f;
    public float wailFearDuration = 1.8f;
    public float wailRadius = 1.8f;

    [Header("movement: speed settings")]
    public float patrolSpeed = 2.1f;
    public float chaseSpeed = 4.1f;
    public float attackMoveLock = 0.33f;
    ...
    private float lastWailTime;
    private float attackLockTimer;
    private bool isDead;
    private bool isLunging;
    private bool isWailing;
    private Coroutine activeAbility;
    private bool IsBusy => activeAbility != null || isLunging || isWailing || attackLockTimer > 0f;
    private enum ActionState { None, Basic, Lunge, Wail }
    ...
    public float debugYOffset = 2.0f;
    ...
    private void Update()
    {
        ...
        if (tgt != null && Vector3.Distance(transform.position, tgt.position) > 15f)
        {
    ...
    private void BuildTree()
    {
        ...
        var canLunge = new ConditionNode(bb, ()=>{ return CanDoLunge(); }, "can_lunge");
        var doLunge = new ActionNode(bb, ()=>{ currentState = "lunge windup"; return DoLunge(); }, "do_lunge");
        var canWail = new ConditionNode(bb, ()=>{ return CanDoWail(); }, "can_wail");
        var doWail = new ActionNode(bb, ()=>{ currentState = "wail windup"; return DoWail(); }, "do_wail");
        ...
        abilitySelector.Add(
            ...
            new Sequence(bb, "lunge_seq").Add(canLunge, doLunge),
            new Sequence(bb, "wail_seq").Add(canWail, doWail),
            moveToTarget
        );
    ...
    private NodeState MoveTowardsTarget()
    {
        ...
        if (attackLockTimer > 0f || isLunging || isWailing || activeAbility != null) return NodeState.Running;
    ...
    private NodeState AttackBasic()
    {
        ...
        Debug.Log($"[TiyanakAI] Starting basic attack on {t.name}");
    ...
    private NodeState DoLunge()
    {
        ...
        activeAbility = StartCoroutine(LungeRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator LungeRoutine()
    {
    ...
    private bool CanDoWail()
    {
        if (IsBusy) return false;
        if (postSpecialTimer > 0f) return false;
        if (isWailing) return false;
        if (Time.time - lastWailTime < wailCooldown) return false;
        var t = bb.Get<Transform>("target");
        if (t == null) return false;
        float d = Vector3.Distance(transform.position, t.position);
        return d <= wailRange + 2f;
    }

    private NodeState DoWail()
    {
        if (IsBusy) return NodeState.Running;
        if (activeAbility != null) return NodeState.Running;
        lastWailTime = Time.time;
        activeAbility = StartCoroutine(WailRoutine());
        return NodeState.Success;
    }

    private System.Collections.IEnumerator WailRoutine()
    {
        currentAction = ActionState.Wail;
        isWailing = true;
        float wind = wailWindup;
        float timeout = abilityHardTimeout;
        ...
        // AOE fear/debuff
        var cols = Physics.OverlapSphere(transform.position, wailRadius, LayerMask.GetMask("Player"));
        foreach (var c in cols)
        {
            var ps = c.GetComponentInParent<PlayerStats>();
            if (ps != null)
            {
                DamageRelay.ApplyToPlayer(ps.gameObject, wailDamage);
    ...
    private void EnsureDebugOverlay()
    {
        ...
        var go = new GameObject("TiyanakDebugText");
    ...
    private void UpdateDebugOverlay()
    {
        ...
        if (debugShowDetails)
        {
            ...
            float cdLunge = LungeCooldownRemaining;
            float cdWail = WailCooldownRemaining;
            details = $"dist:{(dist>=0?dist.ToString("F1"):"-")} lock:{attackLockTimer:F1} lunge:{(isLunging?"Y":"N")} wail:{(isWailing?"Y":"N")}\ncd B:{cdBasic:F1} L:{cdLunge:F1} W:{cdWail:F1}";
        }
