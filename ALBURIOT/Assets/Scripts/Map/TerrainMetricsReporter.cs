...
public class TerrainMetricsReporter : MonoBehaviour
{
    ...
    TerrainSim SimulateTerrain(TerrainGenerator g, int res)
    {
        ...
        // generate falloff
        float[,] fall = GenFalloff(w, h, g.falloffA, g.falloffB);
        // biome and height sampling (low res)
        System.Random prng = new System.Random(12345);
        float offsetX = prng.Next(-100000, 100000);
        float offsetY = prng.Next(-100000, 100000);
        float boX = prng.Next(-100000, 100000);
        float boY = prng.Next(-100000, 100000);
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                float bx = (x + boX) / g.biomeScale;
                float by = (y + boY) / g.biomeScale;
                float biome = Mathf.PerlinNoise(bx, by);
                bool isMount = biome > g.mountainBiomeThreshold;
                int oct = isMount ? g.mountainOctaves : g.octaves;
                float heightMult = isMount ? g.mountainHeightMultiplier : g.heightMultiplier;
                if (isMount) mountain++;

                float nx = (x + offsetX) / g.scale;
                float ny = (y + offsetY) / g.scale;
                float val = FractalNoise(nx, ny, oct, g.persistence, g.lacunarity);
                val = Mathf.Pow(val, g.noisePower);
                if (g.useRidgedNoise) val = 1f - Mathf.Abs(val * 2f - 1f);
                float hval = Mathf.Clamp01(val - fall[x, y]);
                hval *= heightMult / Mathf.Max(0.0001f, g.heightMultiplier);
        ...
        float H = Mathf.Log(Mathf.Max(1e-4f, g.persistence)) / Mathf.Log(Mathf.Max(1.0001f, g.lacunarity));
        float fractalD = 3f - H;
    ...
    SpawnerSim SimulateSpawner(TerrainGenerator g, PerlinEnemySpawner s, int res)
    {
        ...
        if (g != null)
        {
            ...
            var fall = GenFalloff(w, h, g.falloffA, g.falloffB);
            ...
            for (int x = 0; x < w; x++)
            {
                float bx = (x + boX) / (g.biomeScale);
                float by = (y + boY) / (g.biomeScale);
                bool isMount = Mathf.PerlinNoise(bx, by) > g.mountainBiomeThreshold;
                int oct = isMount ? g.mountainOctaves : g.octaves;
                float nx = (x + offX) / g.scale;
                float ny = (y + offY) / g.scale;
                float val = FractalNoise(nx, ny, oct, g.persistence, g.lacunarity);
                val = Mathf.Pow(val, g.noisePower);
                if (g.useRidgedNoise) val = 1f - Mathf.Abs(val * 2f - 1f);
                float hval = Mathf.Clamp01(val - fall[x, y]);
                hval *= (isMount ? g.mountainHeightMultiplier : g.heightMultiplier) / Mathf.Max(0.0001f, g.heightMultiplier);
                heights[x, y] = hval;
            }
        }
