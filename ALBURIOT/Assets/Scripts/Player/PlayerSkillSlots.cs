...
public class PlayerSkillSlots : MonoBehaviour
{
    ...
    if (power != null)
    {
        ...
        // request local input lock if requested (temporary) - uses LocalInputLocker to block movement/camera/combat
        int inputLockToken = 0;
        if (power.stopPlayerOnActivate)
        {
            var locker = LocalInputLocker.Ensure();
            // lock movement only, keep combat/camera unlocked by default for skill activation
            inputLockToken = locker.Acquire("SkillUse", lockMovement: true, lockCombat: false, lockCamera: false, cursorUnlock: false);
            Debug.Log("[PlayerSkillSlots] Acquired input lock for skill activation.");
        }
        ...
        // release input lock after duration if we acquired one
        if (inputLockToken != 0)
        {
            float releaseDelay = power.stopDuration > 0f ? power.stopDuration : 0.5f;
            StartCoroutine(ReleaseInputLockAfter(inputLockToken, releaseDelay));
        }
    ...
    private System.Collections.IEnumerator ReleaseInputLockAfter(int token, float delay)
    {
        yield return new WaitForSeconds(delay);
        LocalInputLocker.Ensure().Release(token);
        Debug.Log("[PlayerSkillSlots] Released input lock after skill activation.");
    }
}
