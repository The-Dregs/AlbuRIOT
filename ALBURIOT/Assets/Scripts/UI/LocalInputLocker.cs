...
public class LocalInputLocker : MonoBehaviour
{
    ...
    private void ApplyStateIfNeeded()
    {
        ...
        if ((_owners.Count > 0) && (!_resolved || playerController == null || playerCombat == null))
        {
            ResolveLocalRefs();
        }
    ...
    private void ResolveLocalRefs()
    {
        ...
        {
            if (Camera.main != null)
                cameraOrbit = Camera.main.GetComponent<ThirdPersonCameraOrbit>();
            if (cameraOrbit == null)
                cameraOrbit = Object.FindFirstObjectByType<ThirdPersonCameraOrbit>();
        }
    }
