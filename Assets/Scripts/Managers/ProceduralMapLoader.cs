...
void Start()
    ...
    private IEnumerator Co_LoadMapAndSpawnPlayer()
    {
        ...
        if (loadingPanel != null && loadingPanel.transform.parent != transform)
        ...
        // clear leftover runtime network objects before scene load to prevent ViewID collisions on joiners
        var cleanupManager = MemoryCleanupManager.EnsureInstance();
        cleanupManager?.CleanupLocalActorPhotonObjectsForTransition();
