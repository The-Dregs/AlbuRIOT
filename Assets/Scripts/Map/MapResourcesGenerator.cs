...
public class MapResourcesGenerator : MonoBehaviourPunCallbacks
{
    ...
    /// </summary>
    private void SyncTerrainSeedToClients()
    {
        if (!PhotonNetwork.IsConnected || !PhotonNetwork.InRoom || !PhotonNetwork.IsMasterClient)
            return;
        ...
        photonView.RPC(nameof(RPC_SyncTerrainSeed), RpcTarget.OthersBuffered, terrainSeed);
        if (logSummary) Debug.Log($"[MapResourcesGenerator] Synced terrain seed {terrainSeed} to clients.");
    }

    [PunRPC]
    private void RPC_SyncTerrainSeed(int terrainSeed)
    {
    ...
    private Transform SpawnCampPrefab(string photonName, GameObject localPrefab, Vector3 position, Quaternion rotation)
    {
        ...
        if (shouldNetwork)
        {
            for (int i = 0; i < candidates.Count; i++)
            {
                ...
                try
                {
                    GameObject go = PhotonNetwork.Instantiate(candidate, position, rotation);
    ...
    private Transform SpawnPrefab(string photonName, GameObject localPrefab, Vector3 position, Quaternion rotation, bool needsNetworking = true)
    {
        ...
        if (shouldNetwork && !string.IsNullOrWhiteSpace(photonName))
        {
            try
            {
                go = PhotonNetwork.Instantiate(photonName, position, rotation);
            }
            catch (System.Exception e)
            {
                Debug.LogError($"[MapResourcesGenerator] Failed to instantiate networked object {photonName}: {e.Message}. Skipping spawn to avoid host/client desync.");
                return null;
            }
        }

        // fallback / offline path: if no network instance was created (or networking not requested),
        // instantiate the local prefab instead. this also covers cases where a Photon prefab name
        // is intentionally left empty (e.g. local-only SpawnArea prefabs).
        if (go == null)
        {
            if (localPrefab != null)
            {
    ...
    private bool CanDestroyNetworkObject(PhotonView pv)
    {
        ...
        return PhotonNetwork.IsMasterClient || pv.IsMine;
    }
    ...
    private void SafeDestroyWithPhotonView(GameObject go)
    {
        ...
        // non-owner clients should ask master to destroy shared runtime views
        if (photonView != null && photonView.ViewID > 0)
            photonView.RPC(nameof(RPC_RequestNetworkDestroy), RpcTarget.MasterClient, pv.ViewID);

        if (logSummary)
            Debug.Log($"[MapResourcesGenerator] non-owner cleanup requested for '{go.name}' (ViewID {pv.ViewID}); waiting for authoritative destroy event.");
    }
